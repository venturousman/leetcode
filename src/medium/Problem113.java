...
public class Problem113 {
	...
	public static void process(TreeNode node, int sum, List<Integer> path, List<List<Integer>> paths) {
		if (node == null)
			return;
		...
		process(node.left, sum - node.val, path, paths);
		process(node.right, sum - node.val, path, paths);
		path.removeLast();
	}
	...
	public static List<List<Integer>> pathSum(TreeNode root, int targetSum) {
		...
		process(root, targetSum, path, paths);
		return paths;
	}
